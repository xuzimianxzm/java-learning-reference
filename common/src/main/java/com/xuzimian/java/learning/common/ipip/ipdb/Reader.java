...
public class Reader {
    ...
    public String[] find(String addr, String language)
            throws IPFormatException, InvalidDatabaseException, UnsupportedEncodingException {
        ...
        return Arrays.copyOfRange(data.split("\t", this.meta.Fields.length * this.meta.Languages.size()), off,
    ...
    private String resolve(int node) throws InvalidDatabaseException, UnsupportedEncodingException {
        ...
        byte b = 0;
        int size = Long.valueOf(bytesToLong(b, b, this.data[resoloved], this.data[resoloved + 1])).intValue();
    ...
    private int readNode(int node, int index) {
        ...
        return Long.valueOf(bytesToLong(this.data[off], this.data[off + 1], this.data[off + 2], this.data[off + 3]))
                .intValue();
    }
